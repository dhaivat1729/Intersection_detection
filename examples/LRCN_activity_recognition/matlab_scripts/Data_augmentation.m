...
root =  '/media/dhaivat1729/Dhaivat666/Oxford_dataset/IROS_sequences/';
dir_root_list = dir(root);
dir_1 = root;

fileID = fopen('image_list.txt','w');
image_paths = [];

save_path = '/media/dhaivat1729/Dhaivat666/Oxford_dataset/IROS_final/';

count = 1;
a = 1;
b = 1;
flag = 0;
for i = 4:(length(dir_root_list))
    dir_1 = root;
...
%             end
            ...
            for l = 3:length(dir_3_list)
                ...
                if i == 3
                    ...
                    imwrite(final_out_1, strcat(save_path, 'Intersection_', num2str(a), '/', dir_3_list(l).name));
                    imwrite(final_out_2, strcat(save_path, 'Intersection_', num2str(a+1), '/', dir_3_list(l).name));
                    imwrite(final_out_3, strcat(save_path, 'Intersection_', num2str(a+2), '/', dir_3_list(l).name));
                    imwrite(final_out_4, strcat(save_path, 'Intersection_', num2str(a+3), '/', dir_3_list(l).name));
                    
                    fprintf(fileID, strcat(save_path, 'Intersection_', num2str(a), '/', dir_3_list(l).name, ' ', ' 0\n'));
                    fprintf(fileID, strcat(save_path, 'Intersection_', num2str(a+1), '/', dir_3_list(l).name, ' ', ' 0\n'));
                    fprintf(fileID, strcat(save_path, 'Intersection_', num2str(a+2), '/', dir_3_list(l).name, ' ', ' 0\n'));
                    fprintf(fileID, strcat(save_path, 'Intersection_', num2str(a+3), '/', dir_3_list(l).name, ' ', ' 0\n'));
                    ...
                    fprintf(fileID, strcat(save_path, 'Non_Intersection_', num2str(b), '/', dir_3_list(l).name, ' ', ' 1\n'));
